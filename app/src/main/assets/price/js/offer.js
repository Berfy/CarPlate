...
//车牌号转换
    /**
     * Created by wlb on 2017/2/22.
     */
    //first step
    // var licenseNo = getQueryStr("vehicleId");
    function picCallback(str1){
        $("#text1").val(str1);
        var licenseNo = "京P5VE87";
        content(licenseNo)
    };
    picCallback();
                   ...
                   // alert(69+vehicleId);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $(".btnof a").attr("href", "../vehicle/vehicleType.html?region=" + region + "&licenseNo=" + encodeURI(licenseNo));
                        }
                    });
                    //功能
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (var i = 0; i < len; i++) {//添加套餐类型
                                if(oData.list[i].package_type=="D"){
                                    continue;
                                }
                                package_name = oData.list[i].package_name;
                                package_des = oData.list[i].package_des;
                                package_type = oData.list[i].package_type;
                                str += '<div class="pro" data-type="' + package_type + '"><h5>' + package_name + '</h5><h5>' + package_des + '</h5></div>'
                            }
                            /*if (len == 3) {
                             $(".pri_pac").addClass('w100').removeClass('w125')
                             } else if (len == 4) {
                             $(".pri_pac").addClass('w125').removeClass('w100')
                             }*/
                            //初始化
                            function init(a) {
                                // if(oData.list.length==4){
                                //     index=a?a:1;
                                // }else{
                                //     index=a?a:0;
                                // }
                                index=a?a:0;
                                $(".pri_pac").attr("data-length",oData.list.length)
                                vehicle = oData.list[index].vehicle;
                                additional = oData.list[index].additional;
                                primary = oData.list[index].primary;
                                ...
                                var vehicle_s2 = '</p> '
                                var vehicle_s3 = '</div></div> </div>' + input_v + ' </div>';
                                ...
                                for (var i = 0; i < primary.length; i++) {
                                    ...
                                    var primary_s2 = primary[i].insuranceName + ' </p>';
                                    var primary_s3 = '</div>  </div> </div>' + exempt +  input_p + ' </div> <div class="opition">' + select_p + '</div></div>'
                                ...
                                for (var i = 0; i < additional.length; i++) {
                                    ...
                                    var additional_s2 = additional[i].insuranceName + '  </p>';
                                    var additional_s3 = '</div>  </div> </div>  ' + exempt + input_a + '</div><div class="opition">' + select_p + '</div></div>';
